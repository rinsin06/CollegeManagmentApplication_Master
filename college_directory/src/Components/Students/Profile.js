...
const Profile = () => {
  ...
  } = useQuery("profileFetch", getStudentProfile, {
    onSuccess: (result) => {
      if (result) {
  ...
  const updateMutation = useMutation(updateStudentProfile, {
    onSuccess: (data) => {
      setIsEditing(false);
    ...
    <Box display="flex" justifyContent="space-between" alignItems="flex-start" padding={2}>
      ...
      <Box flexGrow={1}>
        ...
        <TextField
          ...
          InputLabelProps={{ shrink: true }}
        />
        <Typography variant="body1">Registration Number: {profileData?.userId}</Typography>
